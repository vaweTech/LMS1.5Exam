...
export default function CRTPOAnalyticsDashboardPage() {
  ...
  const stats = useMemo(() => {
    ...
    const totalPlaced = STUDENT_DATA.filter((s) => s.status === "Placed").length;
    ...
    const averagePackage = placedStudents.length
      ? (
          placedStudents.reduce((sum, s) => sum + s.packageLpa, 0) /
  ...
  const kpis = [
    ...
    },
    {
      ...
      value: COMPANY_RECRUITMENT.length,
      icon: Building2,
      accent: CARD_COLORS.indigo,
    },
    {
      label: "Offers Made",
      value: COMPANY_RECRUITMENT.reduce((sum, c) => sum + c.offers, 0),
      icon: TrendingUp,
  ...
  const handleExport = () => {
    const csv = [
      "RegNo,Name,Status,Company,PackageLPA,OfferDate",
      ...filteredStudents.map(
        ({ regNo, name, status, company, packageLpa, offerDate }) =>
          `${regNo},${name},${status},${company},${packageLpa},${offerDate}`
      ),
  ...
  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
    <div>
      ...
      <p className="text-sm text-slate-500 mt-1">
        Monthly placement and offer growth
      </p>
    </div>
  </div>
    ...
    <ResponsiveContainer width="100%" height="100%">
      ...
      >
        ...
        <div className="mt-6 grid grid-cols-1 xl:grid-cols-2 gap-6">
          <TableSection
            title="Placed Students"
            headers={["Reg No", "Name", "Company", "Package"]}
            rows={placedStudents}
            renderRow={(s) => (
              <tr
                key={s.regNo}
