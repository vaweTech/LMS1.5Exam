...
import { ArrowLeft, RefreshCw, PlusCircle, Save } from "lucide-react";
...
export default function ActiveInchargeDashboardPage() {
  ...
  const fetchAssignedClasses = async (u) => {
    const q = firestoreHelpers.query(
      firestoreHelpers.collection(db, "assignedClasses"),
      firestoreHelpers.where("inchargeId", "==", u.uid)
    );
  ...
  const fetchActivities = async (classId) => {
    ...
    const q = firestoreHelpers.query(
      firestoreHelpers.collection(db, "activities"),
      firestoreHelpers.where("classId", "==", classId),
      firestoreHelpers.where("inchargeId", "==", user.uid)
    );
  ...
  useEffect(() => {
    if (selectedClass) {
      fetchActivities(selectedClass.classId || selectedClass.id);
      fetchStudents(selectedClass.classId || selectedClass.id);
    }
  }, [selectedClass]);
  ...
  const createActivity = async () => {
    const cls = assignedClasses.find(
      (c) => c.classId === activityForm.classId
    );
    ...
    if (!activityForm.activityTitle || !activityForm.maxMarks) {
    ...
    fetchActivities(cls.classId || cls.id);
  };
  ...
  const saveResults = async () => {
    ...
    for (const s of students) {
      const data = studentResults[s.id] || {};

      await firestoreHelpers.setDoc(
        firestoreHelpers.doc(
          db,
          "activityResults",
          `${selectedActivity.id}_${s.id}`
        ),
        {
          activityId: selectedActivity.id,
          studentId: s.id,
  ...
  return (
    <div className="p-6">
        ...
        {assignedClasses.map((cls) => (
          <button
            ...
            className={`px-4 py-2 border rounded ${
              selectedClass?.id === cls.id ? "bg-blue-600 text-white" : ""
            }`}
          >
      ...
      {/* CREATE ACTIVITY */}
      <h2 className="text-xl font-bold mb-3">Create Activity</h2>

      <div className="grid grid-cols-2 gap-3 mb-4">
        <select
          value={activityForm.classId}
          onChange={(e) =>
            setActivityForm((p) => ({ ...p, classId: e.target.value }))
          }
          className="border p-2"
        >
          <option value="">Select Class</option>
          {assignedClasses.map((c) => (
            <option key={c.id} value={c.classId || c.id}>
              {c.className}
            </option>
          ))}
        </select>
      ...
      {selectedClass && (
        <>
          <h2 className="text-xl font-bold mt-6 mb-3">
            Activities - {selectedClass.className}
          </h2>
      ...
      {selectedClass && (
        <>
          ...
          {!selectedActivity && <p>Select activity to enter marks</p>}
                ...
                <tr key={s.id}>
                  ...
                  <td>
                    <input
                      type="number"
                      onChange={(e) =>
                        updateResult(s.id, "marks", e.target.value)
                      }
                  ...
                  <td>
                    <select
                      onChange={(e) =>
                        updateResult(s.id, "status", e.target.value)
                      }
                    >
          ...
          {selectedActivity && (
            <button
              onClick={saveResults}
              className="mt-4 bg-green-600 text-white px-4 py-2 flex gap-2"
            >
              <Save /> Save Results
            </button>
          )}
